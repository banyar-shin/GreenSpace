...
def get_unique_output_dir(base_dir):
    counter = 1
    while True:
        unique_dir = os.path.join(base_dir, f"mesh{counter}")
...
for i, image in enumerate(images):
    unique_output_dir = get_unique_output_dir(base_output_dir)
    ...
    if args.render:
        ...
        for ri, render_image in enumerate(render_images[0]):
            render_image.save(os.path.join(unique_output_dir, f"render_{ri:03d}.png"))
        save_video(
            render_images[0], os.path.join(unique_output_dir, f"render.mp4"), fps=30
        )
        timer.end("Rendering")
    ...
    out_mesh_path = os.path.join(unique_output_dir, f"mesh.{args.model_save_format}")
    if args.bake_texture:
        out_texture_path = os.path.join(unique_output_dir, "texture.png")
